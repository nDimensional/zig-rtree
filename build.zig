...
pub fn build(b: *std.Build) !void {
    ...
    addTarget(b, "x86_64-linux/libquadtree.so", .{ .cpu_arch = .x86_64, .os_tag = .linux });
    addTarget(b, "aarch64-linux/libquadtree.so", .{ .cpu_arch = .aarch64, .os_tag = .linux });
    addTarget(b, "x86_64-macos/libquadtree.dylib", .{ .cpu_arch = .x86_64, .os_tag = .macos });
    addTarget(b, "aarch64-macos/libquadtree.dylib", .{ .cpu_arch = .aarch64, .os_tag = .macos });
